...
AUDIO_FEATURES = pd.read_pickle('cs109_final_backend/cluster_files/audio_features.pickle')
...
def make_audio_dict(audio_features):
	audio_scaled = StandardScaler().fit_transform(audio_features)

	audio_dict = {}
	for i in range(len(URIS_SET)):
		audio_dict[URIS_SET[i]] = audio_scaled[i]
...
def convert_to_viz_data(suggestions):
	vis_data = [{'x': URI_TO_SONG[s[0]][:20], 'y': round(float(s[1]), 3)} for s in suggestions if s[0] in URI_TO_SONG]
	...
	suggestion_data = [{'song_name': URI_TO_SONG[s[0]],
						...
						'uri' : s[0]
						} for s in suggestions if s[0] in URI_TO_SONG]
...
def cluster_most_likely(request):
	...
	def predict_knn(playlist, song_to_cluster, cluster_to_songs, audio_dict):
		...
		max_cluster = cluster_to_songs[max_cluster_id]
		np.random.shuffle(max_cluster)

		avg_feat = get_average_features(playlist, audio_dict)
		distances = [(uri, distance(avg_feat, audio_dict[uri])) for uri in max_cluster if uri not in playlist]
		distances.sort(key=lambda tup: tup[1])

		num_to_return = min(15, len(distances))
		return [s for s in distances[:num_to_return]]

	def get_average_features(playlist, audio_dict):
